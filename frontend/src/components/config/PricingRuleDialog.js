...
export default function PricingRuleDialog({ kind, source, open, onOpenChange, onSaved }) {
  ...
  useEffect(() => {
    if (!open) return;
    setForm({ ...EMPTY, ...(source || {}), valid_from: (source?.valid_from || "").slice(0, 10),
        ...
        <div className="max-h-[62vh] space-y-3 overflow-y-auto pr-1">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input id="pr-max" type="number" data-testid={PRICING.formMax}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input id="pr-from" type="date" data-testid={PRICING.formFrom} value={form.valid_from}
                onChange={(e) => patch({ valid_from: e.target.value })} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="pr-until">Berlaku sampai</Label>
              <Input id="pr-until" type="date" data-testid={PRICING.formUntil} value={form.valid_until}
